...
public static String encrypt(String originalText, String passcode) {
		
		String encryptedText = "";
		
		try {
		InputStream input = new ByteArrayInputStream(originalText.getBytes(StandardCharsets.UTF_8));
		ByteArrayOutputStream output = new ByteArrayOutputStream() ;

		encrypt(128, passcode.toCharArray(), input, output);		
		//encryptedText  = DatatypeConverter.printBase64Binary((output).toByteArray());
        encryptedText  = android.util.Base64.encodeToString(output.toByteArray(), 0);
    ...
    public static String decrypt(String encryptedText, String passcode) {
		
		String decryptedText = "";
		
		try {
		
		//InputStream input = new ByteArrayInputStream(DatatypeConverter.parseBase64Binary(encryptedText));

            InputStream input = new ByteArrayInputStream(android.util.Base64.decode(encryptedText, 0));

            ByteArrayOutputStream output = new ByteArrayOutputStream() ;
		
		decrypt(passcode.toCharArray(), input, output);
